...
/*:
 * @target MZ
 * @plugindesc Common base plugin for menu screen standing pictures.
 * @author Lulu's Church
 * @url https://nine-yusha.com/plugin-menuscreen/
 *
 * @help LL_MenuScreenBase.js
 *
 * This is the common base plugin for menu screen standing pictures.
 * Define actor-specific standing picture lists here.
 *
 * You can define multiple standing pictures with state, switch, and variable
 * conditions:
 *   ・Switch1 ON + Poison state
 *   ・Variable1 >= 10 + Poison state
 *   ・Switch1 ON
 *   ・Poison state
 *   ・Normal picture without conditions (required at minimum)
 *
 * Switching by remaining HP%:
 *   First, create a list with "HP%" = "100".
 *   Copy it and set "HP%" = "50" to make another list.
 *   When HP is below half, the "50" list picture is displayed.
 *   Multiple pictures can be defined for different HP%.
 *
 * Display priority order:
 *   1. Matches state ID, switch ID, and variable condition
 *   2. Matches state ID and switch ID
 *   3. Matches state ID and variable condition
 *   4. Matches state ID only
 *   5. Matches switch ID and variable condition
 *   6. Matches switch ID only
 *   7. Matches variable condition only
 *   8. No conditions set (state, switch, variable empty)
 *   (Among these, the lowest HP% takes priority)
 *
 * Integration with battle plugin:
 *   If LL_StandingPictureBattle is installed,
 *   it can integrate directly with the battle standing picture list.
 *
 ...
 * @desc Define standing pictures shown on the menu screen.
 * You can define multiple for specific states or switches ON.
 * @default []
 ...
 * @text Link Standing List
 * @desc Link with LL_StandingPictureBattle standing list.
 * If ON, this plugin’s list is ignored.
 * @default false
 * @type boolean
 * @parent onSpbPlugin
 */
 ...
 * @param stateId
 * @text State ID
 * @desc Use to change picture when actor has specific state.
 * Leave blank for normal picture.
 * @type state
 *
 * @param switchId
 * @text Switch ID
 * @desc Use to change picture when switch is ON.
 * Leave blank for normal picture.
 * @type switch
 *
 * @param variableCase
 * @text Variable Cond.
 * @desc Use to change picture based on variable condition.
 * @default
 * @type struct<variableCase>
 *
 * @param hpPercentage
 * @text HP %
 * @desc Use to change picture by remaining HP%.
 * Normal picture should be set to 100%.
 * @default 100
 * @min 0
 * @max 100
 * @type number
 ...
 * @param x
 * @text X Pos
 * @desc Adjustment value for X position. + moves right, - moves left.
 * Default: 0
 * @default 0
 * @min -9999
 * @max 9999
 ...
 * @param y
 * @text Y Pos
 * @desc Adjustment value for Y position. + moves down, - moves up.
 * Default: 0
 * @default 0
 * @min -9999
 * @max 9999
